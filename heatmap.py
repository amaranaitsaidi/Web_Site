...
df_hour_list = []
for annee_extract in df_copy.annee_extract.sort_values().unique():
    df_hour_list.append(df_copy.loc[df_copy.annee_extract == annee_extract, ['lat', 'lon', 'count']].groupby(['lat', 'lon']).sum().reset_index().values.tolist())
...
base_map.save('heatmap_annee.html')
